...
public:
    ...
    float get_x_min() const { assert(!m_x.empty()); return m_x.front(); }
    float get_x_max() const { assert(!m_x.empty()); return m_x.back(); }
...
std::vector<float> spline::solve(float y, bool ignore_extrapolation) const
{
    ...
    for(size_t i=0; i<n-1; i++) {
        ...
        for(size_t j=0; j<root.size(); j++) {
            ...
            float eps = internal::get_eps()*512.0f*std::min(h,1.0f);
                                ...
                                int newton_iter)
{
    ...
    float p_err = eps*((3.0/3.0)*fabs(b)+(4.0/9.0)*(c*c)+fabs(p));
    float r_err = eps*(6.0*(c*c)+18.0*fabs(b)+fabs(r));
    float q_err = 0.5*fabs(a)*eps + (1.0/54.0)*fabs(c)*(r_err+fabs(r)*3.0*eps)
                   + fabs(q)*eps;
